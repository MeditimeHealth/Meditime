...
export default function CreateDiagnosticCenterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        showToast.success(language === 'bn' ? "সেন্টার সফলভাবে তৈরি হয়েছে" : "Center created successfully");
        router.push("/admin/diagnostic/centers");
        router.refresh();
      } else {
        showToast.error(data.error || "Failed to create diagnostic center");
      }
    } catch (error) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 mt-1 text-lg">
            {language === 'bn' ? 'সিস্টেমে একটি নতুন ডায়াগনস্টিক সেন্টার যোগ করুন' : 'Add a new diagnostic center to the system'}
          </p>
        </div>
      </div>
