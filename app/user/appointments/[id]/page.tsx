...
import { toBengaliNumber } from "@/lib/time-utils";
...
function AppointmentDetailsContent() {
  ...
  const translations = {
    success: { en: "Appointment Details", bn: "অ্যাপয়েন্টমেন্টের বিস্তারিত" },
    confirmed: { en: "Your appointment information is shown below", bn: "আপনার অ্যাপয়েন্টমেন্টের বিস্তারিত তথ্য নিচে দেওয়া হলো" },
    ...
    backToDashboard: { en: "Back to Dashboard", bn: "ড্যাশবোর্ডে ফিরে যান" },
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-12 text-center">
        ...
        <h1 className="text-2xl font-bold text-slate-800 mb-2">
          {language === 'bn' ? 'অ্যাপয়েন্টমেন্ট পাওয়া যায়নি' : 'Appointment Not Found'}
        </h1>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-8 pb-16">
      {/* Success Header - Copy exact design from success page */}
      <div className="text-center mb-8 print:hidden" id="success-header">
                ...
                <div className="min-w-0">
                  ...
                  {(doc.specialtyBn || doc.specialty) && (
                    <p className="text-sm text-[#00B7B5] font-semibold mb-0.5">
                      {language === 'bn' ? (doc.specialtyBn || doc.specialty) : (doc.specialty || doc.specialtyBn)}
                    </p>
                  )}
                  {(doc.qualificationBn || doc.qualification) && (
                    <p className="text-sm text-slate-600 font-medium mb-0.5">
                      {language === 'bn' ? (doc.qualificationBn || doc.qualification) : (doc.qualification || doc.qualificationBn)}
                    </p>
                  )}
                ...
                <div className="space-y-1 p-3 bg-slate-50 rounded-xl">
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${
                      ...
                      appointment.status === 'completed' ? 'bg-blue-50 text-blue-600 border border-blue-100' :
                      'bg-yellow-50 text-yellow-600 border border-yellow-100'
                    }`}
                  >
                    <CheckCircle className="h-3 w-3" />
                    {appointment.status === "pending" ? t.statusPending[language] :
                     appointment.status === "confirmed" ? t.statusConfirmed[language] :
                     appointment.status === "cancelled" ? t.statusCancelled[language] :
      ...
      {/* Print Button - Exact design from success page */}
      <div className="mt-4 flex justify-center print:hidden">
        <Button
          onClick={handlePrint}
          variant="outline"
          className="flex items-center gap-2 hover:border-primary hover:text-primary transition-all rounded-xl font-bold border-slate-200"
        >
      ...
      {/* Important Notes - Exact design from success page */}
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8 print:hidden">
        <Button
          ...
          variant="outline"
          className="w-full sm:w-auto hover:border-primary hover:text-primary transition-all rounded-xl font-bold border-slate-200"
        >
          <ArrowLeft className="h-4 w-4 mr-2" />
        ...
        <Button
          onClick={() => router.push('/')}
          className="w-full sm:w-auto bg-gradient-to-r from-primary to-teal-600 hover:opacity-90 rounded-xl font-bold"
        >
      ...
      <style jsx global>{`
        @media print {
          ...
          /* Hide UI elements */
          nav, footer, aside, .print\\:hidden, #success-header {
          ...
          * {
          ...
          /* Reset main content area - ensure no sidebar margin */
          main {
